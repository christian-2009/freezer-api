...
export class FreezerItemController {
  constructor(private freezerItemService: FreezerItemService) {
    this.freezerItemService = freezerItemService;
  }

  createFreezerItem = async (
    req: Request,
    res: Response,
    next: NextFunction,
  ) => {
    ...
    const result = await this.freezerItemService.createFreezerItem(freezerItem);
    ...
    if (result.type === 'FAILURE') {
      switch (result.failure) {
        case FreezerItemFailureReasons.FAILED_TO_SAVE:
          return res
            .status(500)
            .json({ message: 'Failed to create freezer item' });
      }
    }
